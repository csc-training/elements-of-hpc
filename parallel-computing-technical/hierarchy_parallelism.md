...
We refer to parallelism we refere to the whole suite needed, hardware, compiler and teqniques used in the program to take advantage. 
Processors manufacterers and designers build components which support specific features. 
...
for(int i=0; i<N; i+=k)
    for(int j=0; j<k; j++)
        c[i+j]=a[i+j]+b[i+j];
```
The inner loop in this case can be vectorized by executing for each iteration `i` all the operation corresponding to each value of `j` simultaneously. The size of the vectorization, `k`, depends on the machine, but for small values you can get a `k` times decrease in the execution time. For integers and single precision variables `k=8`.
